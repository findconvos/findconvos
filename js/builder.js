...
editableItems['i'] = ['color', 'font-size'];
editableItems['.editBG, .white-bg, .grey-bg, .light-dark-bg, .deep-dark-bg'] = ['background-image', 'background-color'];
...
$(function(){
	...
	$('#pageList').on("click", ".frameCover > .bgBlock", function(){
		...
		var currentFrameOffWhite = $(this).closest('li').find('iframe').contents().find('.grey-bg');

		var currentFrameLightDark = $(this).closest('li').find('iframe').contents().find('.light-dark-bg');
		var currentFrameDeepDark = $(this).closest('li').find('iframe').contents().find('.deep-dark-bg');

		$(currentFrameWhite).removeClass('white-bg').addClass('grey-bg');
		$(currentFrameOffWhite).removeClass('grey-bg').addClass('white-bg');

		$(currentFrameLightDark).removeClass('light-dark-bg').addClass('deep-dark-bg');
		$(currentFrameDeepDark).removeClass('deep-dark-bg').addClass('light-dark-bg');
	});
	...
	*/
	$('#saveProjectPage').on('click', function(e){
		...
		if (navigator.appVersion.toString().indexOf('.NET') > 0){
			 saveTextAsFile('project.kanebuilder',JSON.stringify(localStorage));
		}else{
			console.save(JSON.stringify(localStorage),'project.kanebuilder');	
		}
